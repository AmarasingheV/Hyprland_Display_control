...
# Mirror or extend displays in a separate thread
def set_display_mode_thread(mode="extend"):
    def task():
        ...
        if mode == "mirror":
            primary = next((m for m in working_monitors if m.get("primary", False)), None)
            if primary is None:
                primary = working_monitors[0]
            res = f"{primary['width']}x{primary['height']}@{primary['refreshRate']}"
            
            # Reset all monitors first (disable all except primary to avoid conflicts)
            ...
            for mon in working_monitors:
                if mon["name"] != primary["name"]:
                    ...
                    time.sleep(0.2)  # Short delay after focus
            ...
            # Re-enable monitors in extended configuration
            x_offset = 0
            for i, mon in enumerate(working_monitors):
            ...
            # Don't call arrange_monitors_thread() here as it conflicts with what we just did
...
def arrange_monitors_thread():
    def task():
        ...
        # Briefly disable all monitors to reset configuration
